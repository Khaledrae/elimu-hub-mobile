...
export default function CourseDetailsScreen() {
  ...
  const loadCourseDetails = async () => {
    try {
      ...
      if (canManageCourse) {
        ...
        const classId = isPremium
          ? selectedClass
          : currentUser.student_profile?.grade_level_id;
        if (classId) {
          lessonsResponse = await courseService.getCourseLessonsByClass(
            courseId,
            classId,
          );
        }
        ...
        const coveredResponse =
          await coveredLessonService.getCourseRecentLessons(
            currentUser.id,
            courseId,
          );
      ...
      const assessmentsResponse =
        await courseService.getCourseAssessments(courseId);

      // Filter assessments based on role
      if (isStudent && currentUser.student_profile?.grade_level_id) {
        // Students only see assessments for their class
        const filteredAssessments =
          assessmentsResponse?.filter((assessment: CourseAssessment) => {
            // Check if assessment's lesson belongs to student's class
            const lesson = lessonsResponse.find(
              (l) => l.id === assessment.lesson_id,
            );
            return (
              lesson?.class_id === currentUser.student_profile?.grade_level_id
            );
          }) || [];
  ...
  const getLessonStatus = (lesson: CourseLesson) => {
    ...
    if (!isPremium) {
      ...
      if (lesson.order > 1 && previousLesson) {
        const previousCovered = coveredLessons.find(
      ...
      if (todayAccessedCount >= DAILY_LESSON_LIMIT) {
        ...
        const isAccessedToday = todayLessons.some(
        ...
        <View style={styles.lessonFooter}>
          <View style={styles.lessonMeta}>
            ...
            <Text style={styles.lessonMetaText}>
              {lesson.content_type.charAt(0).toUpperCase() +
  ...
  const completedCount = coveredLessons.filter(
  ...
  const stickyTabIndex = [
    true, // Header
    isStudent && !isPremium, // Premium Banner
    isStudent, // Progress
    isStudent && isPremium && classes.length > 1, // Class Selector
    true, // Course Header
    true, // Stats
  ].filter(Boolean).length;
  const STICKY_TAB_INDEX = stickyTabIndex - 1;
  return (
        ...
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            ...
            <View style={styles.headerTitleContainer}>
              ...
              <Text style={styles.subtitle}>{getLevelLabel(course.level)}</Text>
            </View>
          </View>
          {canManageCourse && (
            <TouchableOpacity
        ...
        {isStudent && (
          <View style={styles.progressContainer}>
            ...
            <Text style={styles.progressStats}>
              ...
              {!isPremium &&
                ` • ${todayAccessedCount}/${DAILY_LESSON_LIMIT} accessed today`}
            </Text>
          </View>
        )}
